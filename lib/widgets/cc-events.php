...
class CC_Widget extends WP_Widget {
    ...
    function CC_Widget() {
        $this->defaults = array(
            'title' => __('Events/Activities','dbem'),
            'scope' => 'future',
            'order' => 'ASC',
            'limit' => 5,
            'category' => 0,
            ...
            'all_events' => 0,
            'all_events_text' => __('All events/activities', 'dbem'),
            'no_events_text' => __('No events/activities', 'dbem')
        );
        ...
        parent::WP_Widget(false, $name = 'Events/Activities', $widget_ops);    
    }
    ...
    function widget($args, $instance) {
        ...
        $events = EM_Events::get(apply_filters('CC_Widget_events_get_args',$instance));                
        $li_wrap = !preg_match('/^<li>/i', trim($instance['format']));
        if ( count($events) > 0 ){
            foreach($events as $event){                
                if( $li_wrap ){
                    echo '<div class="featured-post">';
                        echo '<h4>'. $event->output("#_EVENTLINK") .'</h4>';                                           
                        if ($event->output("#F #j, #Y") == $event->output("#@F #@j, #@Y"))
                            echo '<div class="post-time">'. $event->output("#F #j, #Y") .'</div>';
                        else
                            echo '<div class="post-time">'. $event->output("#F #j, #Y") . ' - ' . $event->output("#@F #@j, #@Y") .'</div>';
                        echo '<div class="excerpt">'. $event->output("#_EVENTEXCERPT") . '</div>';
                    echo '</div>';
    ...
    function form($instance) {
        ...
        <p>
            ...
            <select id="<?php echo $this->get_field_id('order'); ?>" name="<?php echo $this->get_field_name('order'); ?>">
                <?php 
                $order_options = apply_filters('CC_Widget_order_ddm', array(
...
add_action('widgets_init', create_function('', 'return register_widget("CC_Widget");'));
?>