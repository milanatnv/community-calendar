...
if ($_SESSION["event_exist"] == 1) {
    $_SESSION["event_exist"] = 0;
    $EM_Notices->remove_all();
    $EM_Notices->add_error( '<strong>'.__('Unable to book as another event/activity is already booked.' . $cat1_id,'dbem').'</strong>', true);                
} 
...
<form enctype='multipart/form-data' id="event-form" method="post" action="<?php echo esc_url(add_query_arg(array('success'=>null))); ?>">
	<div class="wrap">
		...
		<?php endif; ?>
		<h3 class="event-form-name"><?php esc_html_e( 'Events/Activities', 'dbem' ); ?></h3>
		<div class="inside event-form-name">
			...
			<br />
			<?php esc_html_e('Example: Loan Closing, Staff Meeting', 'dbem'); ?>
		...
		<!-- START Bookings -->
		<h3><?php esc_html_e('Events/Activities - Bookings/Registration','dbem'); ?></h3>
	...
	<p class="submit">
		<input type="submit" name="events_update" value="<?php esc_attr_e( 'Submit', 'dbem' ); ?> &raquo;" />
	</p>
